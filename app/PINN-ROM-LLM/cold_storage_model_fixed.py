...
class ColdRoomZone:
    ...
    def simulate(
        ...
        for h in range(hours):
            ...
            # Only infiltration exchanges mass/enthalpy with outdoors.
            # Mechanical airflow is internal recirculation (affects h_inner).
            m_dot = AF_infil / 3600.0 * self.rho
            Q_beef_W = task_load * 1000.0

            # Wall conduction (surface-temperature form)
            if rom is not None:
                rom.step_theta(
                ...
                # Steady-wall approximation consistent with the surface method.
            ...
            # ----------------------------------------------------------
            # Sensible energy balance with continuous thermostat model.
            # C dT/dt = U_wall*(T_surf - T) + m_dot*cp*(Tout - T)
            #         + Q_beef - Q_cool
            # When dynamics are fast (tau << dt) the correct hourly
            # cooling energy is the integral of the continuous power
            # needed to hold the setpoint (or the capacity-limited
            # trajectory), not C*DeltaT after an open-loop free response.
            # ----------------------------------------------------------
            U_vent = m_dot * self.cp
            U_tot = U_wall + U_vent
            Q_const = (
                U_wall * T_surf_C
                + U_vent * Tout
            ...
            Q_cool_max_W = max(cooling_capacity_kW, 0.0) * 1000.0

            # Instantaneous heat gain at the setpoint (W)
            Q_gain_at_set = (
                U_wall * (T_surf_C - self.T_set)
                + U_vent * (Tout - self.T_set)
                + Q_beef_W
            )

            if Q_gain_at_set <= 0.0:
                # Free response would stay at or below setpoint – no cooling.
                if a_air > 1e-12:
                    T_eq = Q_const / U_tot
                    decay = math.exp(-a_air * dt)
                    Tin_next = Tin * decay + T_eq * (1.0 - decay)
                else:
                    Tin_next = Tin + (Q_const / self.C_air) * dt
                Q_sensible_kWh[h] = 0.0
            else:
                Q_needed_W = Q_gain_at_set
                if Q_needed_W <= Q_cool_max_W:
                    # Capacity sufficient – hold exactly at setpoint.
                    Tin_next = self.T_set
                    Q_sensible_kWh[h] = Q_needed_W * dt / 3.6e6
                else:
                    # Capacity limited: integrate with constant Q_cool = max.
                    Q_const_lim = Q_const - Q_cool_max_W
                    if a_air > 1e-12:
                        T_eq_lim = Q_const_lim / U_tot
                        decay = math.exp(-a_air * dt)
                        Tin_next = Tin * decay + T_eq_lim * (1.0 - decay)
                    else:
                        Tin_next = Tin + (Q_const_lim / self.C_air) * dt
                    Q_sensible_kWh[h] = Q_cool_max_W * dt / 3.6e6

            # Operating envelope (numerical / capability limits only)
            if Tin_next < T_floor:
            ...
            # ----------------------------------------------------------
            # Moisture: continuous dehumidification to hold RH_set.
            # Only infiltration brings outdoor humidity.
            # ----------------------------------------------------------
            a_w = m_dot / (self.rho * self.V)
            w_set = self.RH_to_w(Tin_next, self.RH_set)

            if a_w > 1e-12:
                # Free moisture response
            ...
            if win_free > w_set:
                # Integrated moisture that must be removed to finish the
                # hour at w_set (exact for the linear free response).
...
def launch_ui():
    if gr is None:
        print("Gradio is not installed. Run with CLI arguments, e.g.:")
        print("  python integrated_cold_storage_fixed.py --mode zone --hours 168 --tag demo")
        return
